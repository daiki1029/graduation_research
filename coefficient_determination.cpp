...
int main()
{
    ...
    FILE *fp_in3 = fopen("バイク拡大画像.bmp", "rb");
    if (fp_in3 == NULL)
    {
        printf("インプットエラー3\n");
        return -1;
    ...
    FILE *fp_out1 = fopen("バイク係数-1.yuv", "wb");
    if (fp_out1 == NULL)
    {
        printf("アウトプットエラー1\n");
        return -1;
    ...
    for (int by = 0; by < PH; by += blockSize)
    {
        for (int bx = 0; bx < PW; bx += blockSize)
        {
            ...
            for (int angle = 0; angle < 180; angle += 10)
            {
                ...
                if ((by / blockSize) == ref_y && (bx / blockSize) == ref_x)
                {
            ...
            if ((by / blockSize) == ref_y && (bx / blockSize) == ref_x)
            {
                ...
                degre = (bestAngle == 0) ? 180 : bestAngle; // 0の場合に180度加算

                // 垂直角度を計算
                int index_vd = (bestAngle / 10 < 9) ? (bestAngle / 10 + 9) : (bestAngle / 10 - 9);
                vertical_degre = eachSum[index_vd];

                std::cout << "垂直の角度は" << index_vd * 10 << "度です。 値は" << vertical_degre << "です。" << std::endl;

                valueRatio = maxSum / vertical_degre;
                cout << "垂直の値との比率は" << valueRatio << "です" << endl;
            }
        }
    }
